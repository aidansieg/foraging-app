...
findsRouter.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!isValidUUID(spot_id)) {
      return res.status(400).json({ error: "spot_id must be a valid UUID" });
    }
    if (!isValidUUID(species_id)) {
      return res.status(400).json({ error: "species_id must be a valid UUID" });
    }
